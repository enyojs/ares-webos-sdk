...
function showUsage() {
	var helpString = [
		"",
		"NAME",
		help.format(processName + " - Provides the debugging Web Inspector."),
		"",
		"SYNOPSIS",
		help.format(processName + " [COMMAND]"),
		help.format(processName + " [OPTIONS] --app, -a <APP_ID>"),
//		help.format(processName + " [OPTIONS]|[COMMAND] --service, -s <SERVICE_ID>"), * TBD *
...
//		help.format("Launch web inspector for <APP_ID> or launch node inspector for <SERVICE_ID> or launch web inspector for web browser"), * TBD *
		help.format("Launch web inspector for <APP_ID>"),
		help.format("This command does not launch an app."),
		help.format("So, an App should be running on the TARGET DEVICE"),
		"",
//		help.format("SERVICE_ID is an service id decribed in services.json"), * TBD *
		help.format("To inspect an app on the TARGET DEVICE,"),
		help.format("user has to specify the TARGET DEVICE using '--device, -d' option"),
		"",
