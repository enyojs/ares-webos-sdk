...
var helpString = [
	"",
	"NAME",
	help.format(processName + " - Manages target device, such as emulator and webOS TV."),
	"",
	"SYNOPSIS",
	help.format(processName + " [COMMAND]"),
	help.format(processName + " --list, -l"),
	help.format(processName + " [OPTIONS] --add, -a <TARGET_INFO>"),
	help.format(processName + " [OPTIONS] --remove, -r <TARGET_INFO>"),
	help.format(processName + " [OPTIONS] --modify, -m <TARGET_INFO>"),
	"",
	"DESCRIPTION",
	help.format("'--list' is used to list up TARGET DEVICE"),
	"",
	help.format("'--add' is used to add TARGET_DEVICE_INFO"),
	...
	"",
	help.format("To remove TARGET_DEVICE_INFO, use '--remove'"),
	help.format("<TARGET_INFO> can be JSON Form."),
	help.format("  (e.g.) --remove '{\"name\": \"tv2\"'}"),
	help.format("Or <TARGET_INFO> can be only NAME of target device"),
	help.format("  (e.g.) --remove tv2"),
	"",
	help.format("To modify TARGET_DEVICE_INFO, use '--modify'"),
	help.format("<TARGET_INFO> can be JSON Form."),
	help.format("  (e.g.) --modify '{\"name\":\"tv2\",\"type\":\"starfish\",\"host\":\"192.168.0.123\",\"port\":\"22\"}'"),
	help.format("Or <TARGET_INFO> can be specified by the additional options."),
	help.format("Please refer to '--add' Usage"),
	"",
	help.format("OPTIONS can be used overlapped, but COMMAND is used isolated."),
	"",
	"OPTIONS",
	help.format("--name, -n [string]"),
	help.format("\t [string] is the device name"),
	"",
	help.format("--type, -t [string]"),
	help.format("\t [string] is the platform type can be 'starfish' or 'emulator'"),
	"",
	help.format("--description, -D [string]"),
	help.format("\t [string] is the description of target device"),
	"",
	help.format("--host, -H [string]"),
	help.format("\t [string] is ip address"),
	"",
	help.format("--port, -p [string]"),
	help.format("\t [string] is a port number"),
	"",
	help.format("--username, -u [string]"),
	help.format("\t [string] is the user name can be 'root' or 'prisoner'"),
	"",
	help.format("--files, -f [string]"),
	help.format("\t [string] is the file stream type can be 'stream' or 'sftp'"),
	help.format("\t if target device support sft-server, sftp is more stable"),
	help.format("\t than general stream"),
	help.format("\t (e.g.) --add --name \"tv2\" --type \"starfish\" "),
	"",
	help.format("--level"),
