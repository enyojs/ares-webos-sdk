...
function PalmGenerate() {
	...
	this.helpString = [
		...
		"",
		"SYNOPSIS",
		help.format(processName + " [OPTIONS] <APP_DIR>"),
		help.format(processName + " [COMMAND]"),
		"",
		"DESCRIPTION",
		help.format("Generate APP in <APP_DIR>"),
		...
		help.format("Surrounding quotes are required in both cases. TEMPLATE is the application template to use."),
		help.format("If not specified, the default template (the firstone marked with `isDefault: true`)."),
		"",
		help.format("OPTIONS can be used overlapped, but COMMAND is used isolated."),
		"",
		"OPTIONS",
		help.format("--template, -t [string]"),
		help.format("\t Use the template named TEMPLATE"),
		help.format("\t TEMPLATE can be searched via " + processName + " --list, -l"),
		"",
		help.format("--property, -p [string]"),
		help.format("\t Set the property PROPERTY"),
		"",
		help.format("--overwrite, -f"),
		help.format("\t Overwrite existing files [boolean]"),
		...
		"",
//		help.format("--proxy-url, -P"),
//		help.format("\t Use the given HTTP/S proxy URL [url]"),
//		"",
		"COMMAND",
		help.format("--list, -l [string]"),
