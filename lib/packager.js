...
(function () {
    ...
    function makeIpk(destination, next) {
		...
		var streams = [ fstream.Reader(path.join(that.tempDir,'control.tar.gz' )) ,
						fstream.Reader(path.join(that.tempDir,'data.tar.gz' )) ] ;
		var ipkStream  = fstream.Writer(path.join(destination, filename)) ;

		arStream.append(header + debBinary);
		streams.forEach( function (s) {
			that.debug("slurp file " + s.basename + " length " + s.size) ;
			console.log(s) ;
			arStream.append(arFileHeader(s.basename, s.size));
			arStream.append(s);
		});

		arStream.pipe(ipkStream) ;

		ipkStream.on('end', function() {
