...
(function () {
    ...
    function fillPackageDir(next) {
        ...
        shelljs.echo(JSON.stringify(pkginfo, null, 2) + "\n").to(path.join(this.packageDir, "packageinfo.json"));
        next();         // TODO: TBC
    }
    ...
    function makeDataTar(next) {
        ...
        if (this.nativecmd) {           // TODO: TBR
            ...
            shelljs.exec("tar cvzf " + path.join(this.tempDir, 'data.tar.gz') + " ./usr");
            next();
            return;
        }
    ...
    function createControlFile(next) {
        ...
        shelljs.echo(ctrl).to(path.join(this.tempDir, "control"));
        next();
    }
    ...
    function createDebianBinary(next) {
        ...
        var ctrl = "2.0\n";
        shelljs.echo(ctrl).to(path.join(this.tempDir, "debian-binary"));
        next();
    }
    ...
    function makeControlTar(next) {
        ...
        if (this.nativecmd) {           // TODO: TBR
            shelljs.cd(this.tempDir);
            shelljs.exec("tar cvzf " + path.join(this.tempDir, 'control.tar.gz') + " ./control");
            next();
            return;
        }
    ...
    function makeIpk(next) {
        this.debug("makeIpk");

        if (this.nativecmd) {           // TODO: TBR
            shelljs.cd(this.tempDir);
            var filename = this.appinfo.id + "_" + this.appinfo.version + "_all.ipk";
            shelljs.exec("ar -q " + path.join(this.tempDir, filename) + " control.tar.gz data.tar.gz debian-binary");
            next();
            return;
        }
