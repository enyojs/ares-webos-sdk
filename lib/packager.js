...
(function () {
    ...
    function changeAppInfo(next) {
        ...
        for (var i in this.appinfo) {
            if (this.appinfo.hasOwnProperty(i) && defaultAssetsFields[i]) {
                ...
                if ((this.appinfo.assets.indexOf(this.appinfo[i]) === -1) &&
                    this.appinfo[i] &&
                    fs.existsSync(path.join(this.appDir, this.appinfo[i]))) {
    ...
    function minifyApp(next) {
        ...
        if (this.minify === true) {
            ...
            if (fs.existsSync(deployJs)) {
                ...
                // If we need to build resolution-indpendent CSS, invoke the proper deploy script argument
                if (this.appinfo.resolutionIndependent) {
                    console.log("Enabling resolution-independence...");
                    deployArgs.push("-ri");
                }
    ...
    function copyAssets(next) {
        ...
        function _copyAssets(file, next) {
            ...
            var destination = path.dirname(path.join(this.applicationDir, file));
            async.series([
                function(next) {
