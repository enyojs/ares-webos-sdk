...
(function () {
    ...
    function changeAppInfo(next) {
        log.verbose("changeAppInfo");
        // contruct appinfo.assets to have default  values so that they can be copied into the package
    ...
    function minifyApp(next) {
        ...
        if (this.minify === true) {
            ...
            if (fs.existsSync(deployJs)) {
                ...
                // If we're specifying a deployscript in an alternative enyo dir, ensure lib still points to the app
    ...
    function copyAssets(next) {
        ...
        function _copyAssets(file, next) {
            ...
            if ((path.resolve(file) == path.normalize(file)) ||
                (path.resolve(file).indexOf(this.originAppDir) !== 0)) {
                isAbsPath = true;
                source = file;
                return next(new Error("In appinfo.json, '" + file + "'' path must be relative to the appinfo.json and be located in application directory."));
            } else {
                source = path.join(this.originAppDir, file);
            }

            if (!fs.existsSync(source)) {
                return next(new Error("'" + file + "'' file does not exist. please check the file path."));
            }

            if (path.resolve(file).indexOf(this.originAppDir) === 0) {
                if (isAbsPath === true) {
                    var subPath = path.relative(this.originAppDir, file);
                    destination = path.dirname(path.join(this.applicationDir, subPath));
                } else {
                    destination = path.dirname(path.join(this.applicationDir, file));
                }
            } else {
                log.verbose("copyAssets # '" + file + "' will be located in app directory");
                destination = this.applicationDir;
            }
    ...
    function excludeFromApp(next) {
        ...
        var excludeList = [];
        var excludes = this.excludeFiles.concat(this.appinfo.exclude || []);
    ...
    function fillPackageDir(next) {
        ...
        if (!this.rom) {
            // Copy icon file
            var filename = path.join(this.applicationDir, this.appinfo.icon);
    ...
    function checkDirectory(options, directory, callback) {
        ...
        if (fs.existsSync(path.join(directory, "appinfo.json"))) {      // TODO: move to asynchronous processing
            ...
            var svcDirs = [];
            this.svcDir = directory;
            findServiceDir.call(this, svcDirs, function(err) {
                if (svcDirs.length > 0) {
                    callback();
                } else {
    ...
    function findServiceDir(services, next) {
        var checkDir = this.svcDir || this.originAppDir;
        _walkFolder(checkDir, function(err){
            setImmediate(next, err);
        }.bind(this));

        function _walkFolder(dirPath, next) {
            async.waterfall([
                fs.readdir.bind(null, dirPath),
                function(fileNames, next) {
                    async.forEach(fileNames, function(fileName, next) {
                        var filePath = path.join(dirPath, fileName);
                        async.waterfall([fs.lstat.bind(null, filePath),
                            function(stat, next) {
                                if (stat.isFile()) {
                                    if (fileName === "services.json") {
                                        var svc = new Service();
                                        svc.srcDir = dirPath;
                                        svc.dirName = path.basename(dirPath);
                                        services.push(svc);
                                    }
                                    next();
                                } else if (stat.isDirectory()) {
                                    _walkFolder(filePath, next);
                                } else {
                                    next();
                                }
                            }
