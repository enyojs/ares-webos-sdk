...
(function () {
    ...
    function decidePkgName(next) {
        if (this.appCount !== 0) {
            ...
            this.pkg = {
                name : this.services[0].serviceInfo.id || this.services[0].serviceInfo.services[0].name,
                version : "1.0.0"
            };
        } else {
            this.pkg = {
    ...
    function makeIpk(destination, next) {
        var filename = this.pkg.name + "_" + this.pkg.version + "_all.ipk";
