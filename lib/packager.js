...
(function () {
    ...
    Packager.prototype = {
        ...
        generatePackage: function(inDirs, destination, options, next) {
            ...
            async.series([
                    ...
                    makeTgz.bind(this,'usr','Directory','data.tar.gz'),
                    createControlFile.bind(this),
                    makeTgz.bind(this,'control','File','control.tar.gz'),
    ...
    function createControlFile(next) {
        ...
        fs.writeFile(path.join(this.tempDir, "control"), lines.join("\n"), next);
    }
    ...
    function makeTgz(input,type,output,next) {
        this.debug("makeTgz " + output + " in " + this.tempDir);
		...
		var inPath = path.join(this.tempDir, input) ;

		fstream
			.Reader( {path: inPath, type: type }  )
