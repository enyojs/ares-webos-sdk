...
(function () {
    ...
    function copySrcToDst(src, dst, next) {
        ...
        async.series([
            ...
            _copySrcToDst.bind(null, fileList, dst)
        ], function(err) {
            next(err);
        });
        ...
        function _copySrcToDst(fileList, dstPath, next) {
            try {
                    ...
                    function(file, next) {
                        ...
                        if (file.type === FILE_TYPE.symlink) {
                            if (file.isSubPath && file.indRelPath) {
                                if (fs.existsSync(path.join(file.basePath, file.indRelPath))) {
                                    fs.symlinkSync(file.indRelPath, path.join(dstPath, file.relPath), null);
                                }
                            }
                        } else {
                            if (fs.existsSync(path.join(file.basePath, file.relPath))) {
                                shelljs.cp('-Rf', path.join(file.basePath, file.relPath), path.join(dstPath, file.relPath, '..'));
                            } else {
