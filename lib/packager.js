...
(function () {
    ...
    function copyService(next) {
		//* copy service files into each serviceinfo[x].id directory.
		for(var idx in this.services) {
			if(this.services[idx].valid === false)
				continue;
    ...
    function addServiceInPkgInfo(next) {
		...
		var serviceNames = this.services.filter(function(s) {return s.valid;});
		validServiceCount = serviceNames.length;
