...
(function () {
    ...
    function copySrcToDst(src, dst, next) {
        try {
            if (process.platform === 'win32') {
                var subProcess = spawn('xcopy', ['/y','/e','/k','/h', src, dst]);
                subProcess.on('exit', function(code) {
                    if (code !== 0) {
                        next("ERROR: xcopy failed");
                        return;
                    }
                    next();
                }.bind(this));
    ...
    function loadPackageProperties (next) {
        this.debug("loadPackageProperties");
        var filename = path.join(this.appDir, "package.properties")
        fs.readFile(filename, function(err, data) {
            try {
                this.debug("PACKAGE PROPERTIES >>" + data + "<<");            
                ...
                for(i in lines) {
                    if(lines[i].indexOf("filemode.") == 0) {
                        ...
                        this.packageProperties[lines[i].substr(seperatorIndex + 1).trim()] = lines[i].slice(9, seperatorIndex).trim();
                    }
                }
                next();
            } catch (err) {
            ...
            try {
                copySrcToDst(path.join(service.srcDir, '*'), service.dstDir, 
                    function(error, stdout, stderr){	
                        if (error) {
                            next(new Error("copyService:",error));
                        } else {
                            completed++;
