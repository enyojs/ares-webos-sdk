        ...
        if (this.minify === true) {
            ...
            if (fs.existsSync(deployJs)) {
                ...
                if (this['deploy-enyo']) {
                    ...
                    //FIXME: this code depends on deploy.js & minify.js (WORKAROUND FOR splitted structure)
                    if (fs.existsSync(path.join(this.appDir, 'enyo'))) {
                        deployArgs.push("-e", 'enyo');
                    }
                }
                if (this['deploy-lib']) {
                    ...
                    //FIXME: this code depends on deploy.js & minify.js (WORKAROUND FOR splitted structure)
                    if (fs.existsSync(path.join(this.appDir, '../lib'))) {
                ...
                if (!this['deploy-srcroot']) {
                    if (!fs.existsSync(path.join(this.appDir, 'deploy.json')) && !fs.existsSync(path.join(this.appDir, 'index.html'))) {
                        if (path.dirname(path.resolve(this.appinfo.main)) !== path.dirname(path.resolve(this.appDir))) {
                            console.log("Set source code root directory :", path.dirname(path.resolve(this.appinfo.main)));
                            this['deploy-srcroot'] = path.dirname(path.resolve(this.appinfo.main));
                        }
                    }
                }
                if (this['deploy-srcroot']) {
                    deployArgs.push("-s", path.relative(path.resolve(this.appDir), path.resolve(this['deploy-srcroot'])));
                }
                ...
                if (this.appinfo.onDeviceSource) {
                    for (var from in this.appinfo.onDeviceSource) {
                        ...
                        //if lib is specified, from should be replace
                        if (from.indexOf('lib'+path.sep) !== -1 && this['deploy-lib']) {
                            subFrom = from.substring(from.indexOf('lib'+path.sep)+4);
                            from = path.relative(this.appDir, path.join(this['deploy-lib'],subFrom));
                            from = from.replace(/[/|\\]/g, path.sep);
                        }
                ...
                subProcess.on('exit', function(code) {
                    ...
                    var destiName = (this['deploy-srcroot'])? path.basename(path.resolve(this['deploy-srcroot'])) : path.basename(this.appDir);
                    var desti = path.join(this.appDir, 'deploy', destiName);
