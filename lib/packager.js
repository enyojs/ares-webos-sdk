    ...
    function minifyApp(next) {
        ...
        if (this.minify === true) {
            ...
            if (fs.existsSync(deployJs)) {
                ...
                if (this['deploy-enyo']) {
                    deployArgs.push("-e", path.relative(path.resolve(this.appDir),path.resolve(this['deploy-enyo'])));
                }
                if (this['deploy-lib']) {
                    deployArgs.push("-l", path.relative(path.resolve(this.appDir), path.resolve(this['deploy-lib'])));
                }
                //FIXME: WORK-AROUND for inproper-bootplate structure 
                //  FOO_BOOTPLATE/
                //          appinfo.json
                //        +-----  enyo/
                //        +-----  lib/
