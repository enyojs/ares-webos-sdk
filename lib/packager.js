...
(function () {
    ...
    function minifyApp(next) {
        ...
        if (this.minify === true) {
            ...
            if (fs.existsSync(deployJs)) {
                var subProcess = spawn(deployJs, [], {cwd: this.appDir});

                var echoStream = function(name, data) {
                    this.debug("MINIFIER " + name + ": " + data);
                };

                subProcess.stderr.on('data', echoStream.bind(this, "SDTOUT"));
                subProcess.stdout.on('data', echoStream.bind(this, "STDERR"));

                subProcess.on('exit', function(code) {
                    if (code !== 0) {
                        next("ERROR: pb during minification");
                        return;
                    }
    ...
    function makeDataTar(next) {
        ...
        var files = shelljs.ls('-R', path.join(this.tempDir, 'usr'));       // TODO: move to asynchronous processing
		...
		// forEachSeries is now name eachSeries starting from async 0.2.5
        async.forEachSeries(files, function(file, cb) {
            file = path.join('./usr', file);
            var stats = fs.statSync(file);                              // TODO: move to asynchronous processing
            if (String(file).length > 99) {
                tooLong.push(file);
                cb();
			}
                ...
                this.debug("Tarball Skipping directory: " + file);
                cb();
            }
			...
			if (tooLong.length) {
                console.log("WARNING: skipped files with long name:\n * " + tooLong.join("\n * "));
			}
