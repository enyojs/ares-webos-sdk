                ...
                if (!this['deploy-srcroot']) {
                    if (!fs.existsSync(path.join(this.appDir, 'deploy.json')) && !fs.existsSync(path.join(this.appDir, 'index.html'))) {
                        if (path.dirname(path.resolve(this.appinfo.main)) !== path.dirname(path.resolve(this.appDir))) {
                            var rootPath = path.dirname(path.resolve(path.join(this.appDir, this.appinfo.main)));
                            console.log("Set source code root directory :", rootPath);
                            this['deploy-srcroot'] = rootPath;
                        }
                    }
                }
                if (this['deploy-srcroot']) {
