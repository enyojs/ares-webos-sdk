...
(function () {
    ...
    var onDeviceSourceAliases = {
        "$enyo-framework": "/dev/enyo/github/device-releases/build"
    };
    ...
    function rewriteEnyoPaths(next) {
        this.debug("rewriteEnyoPaths");
        var enyoPath = this.appinfo.onDeviceSource["enyo"];
        if (enyoPath) {
            try {
                var file = path.join(this.applicationDir, this.appinfo.main);
                var data = fs.readFileSync(file, "utf8");
                var regex = new RegExp("(<link[^>]*['\"])[^'\"]*/enyo.css(['\"])");
                data = data.replace(regex, "$1" + enyoPath + "/enyo.css" + "$2");
                regex = new RegExp("(<script[^>]*['\"])[^'\"]*/enyo.js(['\"])");
                data = data.replace(regex, "$1" + enyoPath + "/enyo.js" + "$2");
