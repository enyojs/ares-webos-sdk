    ...
    function excludeRedundantDirFromApp(next) {
        ...
        if (this.minify !== true) {
            this.excludeFiles = this.excludeFiles.concat([
                path.join(this.applicationDir, 'build'),
                path.join(this.applicationDir, 'target'),
                path.join(this.applicationDir, 'deploy')
            ]);
        } else {
            this.excludeFiles = this.excludeFiles.concat([
                path.join(this.applicationDir, 'target')
            ...
            try {
                ...
                if (fs.existsSync(buildDir)) {
                    fs.readdirSync(buildDir).forEach(function(fileName) {
                        ...
                        fs.writeFileSync(file, data.replace(/[\\]/g, "/"), "utf8");
                    });
                }
            } catch(err) {
                setImmediate(next, err);
        ...
        if (fs.existsSync(filename)) {
            fs.readFile(filename, function(err, data) {
                try {
                    ...
                    this.excludeFiles = this.excludeFiles.concat([
                        path.join(this.applicationDir, "package.properties")
                    ]);
                    setImmediate(next);
                } catch (err) {
                    setImmediate(next, err);
        ...
        function _walkFolder(dirPath, next) {
            async.waterfall([
                ...
                function(fileNames, next) {
                    async.forEach(fileNames, function(fileName, next) {
                        var filePath = path.join(dirPath, fileName);
                        async.waterfall([fs.stat.bind(null, filePath),
                            function(stat, next) {
                                if (stat.isFile()) {
    ...
    function checkServiceInfo(next) {
        ...
        this.services.forEach(function(service) {
            if (service.valid === false)
                return;
            if (!service.serviceInfo.services)
                return;
            var serviceNames = (service.serviceInfo.services instanceof Array) ?
                service.serviceInfo.services : [service.serviceInfo.services];
            serviceNames.forEach(function(serviceName) {
                //serviceName should start with appinfo.id
                if (serviceName.name.indexOf(appId+".") === -1) {
                    var errMsg = "service name \"" + serviceName.name + "\"" +
    ...
    function createServiceDir(next) {
        ...
        this.services.forEach(function(service) {
            ...
            var serviceNames = (service.serviceInfo.services instanceof Array) ?
                service.serviceInfo.services : [service.serviceInfo.services];
            serviceNames.forEach(function(serviceName) {
                var serviceDir = path.join(this.tempDir, "data/usr/palm/services", serviceName.name);
        ...
        if (!this.rom) {
            ...
            var validServices = this.services.filter(function(s) {
                ...
                var serviceNames = (service.serviceInfo.services instanceof Array) ?
                                    service.serviceInfo.services : [service.serviceInfo.services];
                serviceNames.forEach(function(serviceName){
                    pkgServiceNames.push(serviceName.name);
                    validServiceCount++;
                });
            });
