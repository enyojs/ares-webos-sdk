...
(function () {
    ...
    function loadAppInfo(next) {
        ...
        fs.readFile(filename, function(err, data) {
            try {
				...
				// FIXME: display more detail on JSON error (at least line where the error was found)
    ...
    function minifyApp(next) {
        ...
        if (this.minify === true) {
			...
			// FIXME: what if deploy.js does not exist ?
        }
    ...
    function createCtrlDir(next) {
        ...
        this.debug("ctrl dir = " + this.ctrlDir);
			// FIXME: add try/catch
        mkdirp(this.ctrlDir, next); // FIXME:
    }
