...
(function () {
    ...
    function copySrcToDst(src, dst, next) {
        ...
        async.series([
            ...
            _copySrcToDst.bind(null, fileList, src, dst)
        ], function(err) {
            next(err);
        });
            ...
            async.waterfall([
                ...
                function(fileNames, next) {
                    if (fileNames.length === 0) {
                        fileList.push( {
                            isDir: true,
                            relPath: path.relative(basePath, dirPath)
                        });
                        return setImmediate(next);
                    }
        ...
        function _copySrcToDst(fileList, basePath, dstPath, next) {
            try {
                async.forEachSeries(fileList,
                    function(file, next) {
                        ...
                        if (file.isSymlink && file.isSubPath) {
                            fs.symlinkSync(file.indRelPath, path.join(dstPath, file.relPath), null);
                        } else if (!file.isSymlink){
                            if (fs.existsSync(path.join(basePath, file.relPath))) {
                                shelljs.cp('-Rf', path.join(basePath, file.relPath), path.join(dstPath, file.relPath, '..'));
                            } else {
    ...
    function copyAssets(next) {
        ...
        function _copyAssets(file, next) {
            ...
            async.series([
                ...
                copySrcToDst(source, destination, next);
            });
        }
    }
