...
(function () {
    ...
    function linkOnDeviceSources(next) {
        this.debug("linkOnDeviceSources");
        if (this.appinfo.onDeviceSource) {
            for (var from in this.appinfo.onDeviceSource) {
                var to = this.appinfo.onDeviceSource[from];
                var appFrom = path.join(this.applicationDir, from);
                try {
                    shelljs.rm('-rf', appFrom);
                    fs.symlinkSync(to, appFrom, "dir");
                    // Special-case handling for Enyo: symlink built source
                    if (from == "enyo") {
                        // Symlink built JS if exists
                        from = path.join(this.applicationDir, "build/enyo.js");
                        if (fs.existsSync(from)) {
                            shelljs.rm('-rf', from);
                            fs.symlinkSync(to + "/enyo.js", from, "file");
                        }
                        // Symlink built CSS if exists
                        from = path.join(this.applicationDir, "build/enyo.css");
                        if (fs.existsSync(from)) {
                            shelljs.rm('-rf', from);
                            fs.symlinkSync(to + "/enyo.css", from, "file");
                        }
                    }
                } catch(err) {
                    next(err);
    ...
    function outputPackage(destination, next) {
        if (this.rom) {
            shelljs.cp('-R', path.join(this.tempDir, "data/*"), destination);
            next();
        } else {
            async.series([
