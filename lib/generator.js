...
(function () {
    ...
    Generator.prototype = {
        ...
        generate: function(templateId, substitutions, destination, options, callback) {
            ...
            function processZipFile(item, next) {
                ...
                this.addTemplateType = (options && options.hasOwnProperty('add'))? options.add : null;

                // if zip files has 'type' Property then it proceed only when it has a 'add' option.)
                if (item.hasOwnProperty('type') && item.type !== this.addTemplateType) {
                    next();
                }
    ...
    function prefix(item, srcDir, dstDir, options, next) {
        ...
        if (this.existed && !this.overwrite && fs.existsSync(dst)) {
            ...
            if (item.hasOwnProperty('type') && item.type === 'service') {
