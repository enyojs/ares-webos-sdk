...
(function () {
    ...
    function parseInsertTemplates(data, templatesUrl, callback) {
        try {
			...
			// FIXME: add file name and line number or URL and some context
    ...
    function unzipFile(item, destination, options, next) {
		...
		// FIXME: where's the error handler for bad zip files ?
    }
            ...
            shell.ls('-R', destination).forEach(function(file) {
                ...
                item.excluded.forEach(function(pattern) {
                    ...
                    if (regexp.test(file)) {
						...
						// FIXME use rimraf , error handler
                    }
                });
            });
    ...
    applyJsonSubstitutions = function(filename, values) {
        var modified = false;
        var content = shell.cat(filename); // FIXME trap missing file
        content = JSON.parse(content); // FIXME trap bad json
