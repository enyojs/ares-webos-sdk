		...
		inspect:function(options, params, next){
			...
			function _runServicePortForwardServer(next) {
				...
				if (options.serviceId) {
					...
					var cmdRunSvcDbg = sprintf(format, dbgPort, path.join(defaultServiceInstallPath, options.serviceId));
					async.waterfall([
						function(next) {
							//FIXME: need to change to use __quit
							var cmdKillPrvSvc = "kill -9 `pidof " + options.serviceId + ".js`";
