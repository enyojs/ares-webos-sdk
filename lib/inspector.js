				...
				function __eachServicePortForward(serviceId, next) {
					...
					async.waterfall([
						function parserMeta(next) {
							var metaFilePath = path.join(servicePath, "services.json").replace(/\\/g, "/");
							...
							async.series([
								...
								if (err) {
									return next(new Error("webOS Service named '" + serviceId + "' was not installed"));
								}
							});
						...
						function runService(next) {
							...
							if (options.session.getDevice().username == 'root') {
								var cmdRunSvcDbg = util.format(format, dbgPort, path.join(defaultServiceInstallPath, serviceId));
