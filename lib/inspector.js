	...
	var inspector = {
		...
		inspect:function(options, params, next){
			...
			function _runAppPortForwardServer(next){
				if (options.appId && !options.noPortForwarding /*FIXME : delete this second condition when 9998 port is not working directly */) {		
				...
				function __eachServicePortForward(serviceId, next) {
					...
					async.waterfall([
						function(next) {
							...
							var cmdKillPrvSvc = "kill -9 `pidof " + serviceId + "`";
							next(null, cmdKillPrvSvc);
						}.bind(this),
						options.session.runNoHangup.bind(options.session),
			...
			function _runAppInspector(next) {				
				if (options.appId) {		
					...
					/*FIXME : delete this condition when 9998 port is not working directly */
