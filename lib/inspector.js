	...
	var inspector = {
		...
		inspect: function(options, params, next) {
				...
				function __eachServicePortForward(serviceId, next) {
					...
					async.waterfall([
							...
							if (options.instPkgs) {
								options.instPkgs.every(function(pkg) {
									if (serviceId.indexOf(pkg.id) !== -1) {
										options.svcDbgInfo[serviceId]['path'] = path.join(path.dirname(pkg.folderPath), '..', 'services', serviceId).replace(path.sep, '/');
										return false;
									}
									return true;
								});
						...
						function parserMeta(next) {
							var metaFilePath = path.join(options.svcDbgInfo[serviceId]['path'], "services.json").replace(path.sep, '/');
			...
			function _runServiceInspector(next) {
				var svcIds = Object.keys(options.svcDbgInfo);
				async.forEach(svcIds, __launchServiceInspector, function(err) {
					next(err);
				});

				function __launchServiceInspector(svcId, next) {
