...
(function() {
	...
	var installer = {
		...
		install: function(options, hostPkgPath, next) {
			...
			// FIXME: make sure that hostPkgPath does not return empty basename
			...
			async.waterfall([
				...
				fs.readFile.bind(null, hostPkgPath), // FIXME: trap file not found (typo error)
				function(data, next) {
				...
				log.verbose("installer#waterfall callback ", err);
			...
			async.series([
				...
				luna.send.bind(null, options, {
					...
					if (lineObj.status.match(/^FAILED_/)) {
						...
						// success: stop
						next(null, {status: lineObj.status});
					} else {
						// no err & no status : continue
				...
				if ( ! err) {
					result.msg = 'Removing package ' + packageName;
				}
				next(err, result);
			});
		},
