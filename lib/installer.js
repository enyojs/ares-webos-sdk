		...
		install: function(options, hostPkgPath, next) {
			...
			async.waterfall([
				...
				function(status, next) {
					//FIXME: @kiwook, @LG-member debug this, starfish cause problem when running this.
				//	if (options.session.getDevice().name !== "tv")
				//	{
			...
			async.waterfall([
				...
				function(session, next) {
					...
					luna.send(options, addr, param, function(lineObj, next) {
						...
						var resultValue = result.getResultValue(lineObj);
						if (resultValue.match(/^FAILED_/i)) {
							// failure: stop
		...
		list: function(options, next) {
			...
			async.waterfall([
				...
				function(session, next) {
					...
					var result = session.getDevice().lunaAddr.result;
					var param = {
							// luna param
							subscribe: false
						     };	
