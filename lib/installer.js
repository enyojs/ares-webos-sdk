...
(function() {
	...
	var installer = {
		...
		install: function(options, hostPkgPath, next) {
			...
			var prefixPath = '/media/developer/temp/';
			if (!hostPkgName) {
				next(new Error("Invalid package: '" + hostPkgPath + "'"));
				return;
			}
			var devicePkgPath = prefixPath + hostPkgName,
			...
			async.waterfall([
				...
				function(session, next) {
					...
					options.session.run('/bin/rm -rf ' + prefixPath + '&& /bin/mkdir -p ' + prefixPath, null, null, null, next);
				},
				function(next) {
