...
(function () {
	...
	function Session() {
		...
		this.ssh.connect({
		 ...
		 /*
		// Ubuntu Linux Guest, running dropbear on vboxnet0
		this.ssh.connect({
			host: '192.168.56.101',
			port: 22,
			username: 'root',
		 ...
		 */
		/*
		 // Remote machine on the same LAN
		 this.ssh.connect({
		 host: '192.168.1.15',
		 port: 22,
		 username: 'fxk',
	...
	novacom.Session.prototype = {
		...
		_put: function(outPath, inStream, next) {
			...
			this.ssh.exec(cmd, (function(err, stream) {
				log.verbose('Session#_put()', 'exec cmd: ' + cmd + ', err:' + err);
				if (err) {
					next(err);
					return;
				}
				stream.on('data', function(data, extended) {
					log.verbose('Session#_put()', 'os: data');
					log.info('Session#_put()', "(" + extended + ") " + data.toString());
				});
				stream.on('exit', function(code, signal) {
					log.verbose('Session#_put()', 'os: exit :: code=' + code + ', signal=' + signal);
					err = makeExecError(cmd, code, signal);
				});
				stream.on('close', function() {
					log.verbose('Session#_put()', 'os: close, arguments=' + util.inspect(arguments));
					next(err);
				});
				inStream.on('end', function() {
					log.verbose('Session#_put()', 'is: end');
					stream.end();
				});
		...
		_get: function(inPath, outStream, next) {
			...
			this.ssh.exec(cmd, (function(err, stream) {
				log.verbose('Session#_get()', 'exec cmd: ' + cmd + ', err:' + err);
				if (err) {
					next(err);
					return;
				}
				stream.on('data', function(data, extended) {
					log.verbose('Session#_get()', 'is: on data');
					if (extended === 'stderr') {
				...
				stream.on('end', function() {
					log.verbose('Session#_get()', 'is: on end');
				});
				stream.on('exit', function(code, signal) {
					log.verbose('Session#_get()', 'is: on exit code=' + code + ', signal=' + signal);
					err = makeExecError(cmd, code, signal);
				});
				stream.on('close', function() {
					log.verbose('Session#_get()', 'is: on close');
					next(err);
				});
			}).bind(this));
		},
		...
		_run: function(cmd, stdin, stdout, stderr, next) {
			...
			this.ssh.exec(cmd, (function(err, stream) {
				...
				stream.on('data', function(data, extended) {
					log.verbose('Session#_run()', 'is: on data');
					if (extended === 'stderr') {
						stderr.write(data);
					} else {
						stdout.write(data);
				...
				stream.on('close', function() {
					log.verbose('Session#_run()', 'ssh: close');
					next(err);
				});
