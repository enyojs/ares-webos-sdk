	...
	novacom.Resolver.prototype = {
		...
		_loadOne: function(inDevice, next) {
			...
			if (typeof inDevice.privateKey === 'string') {
				...
				var keyDirPath = keydir;
				if (inDevice.host.toString() === '127.0.0.1' && inDevice.port.toString() === '6622') {
					keyDirPath = __dirname;
				} else if (inDevice.password) {
					keyDirPath = __dirname; //This is a wark around to use password authentication for tv
				}
				async.waterfall([
					function(next) {
					...
					if (err) {
						log.verbose("Resolver#_loadOne()", "Unable to find OpenSSH private key named '" + inDevice.privateKey.openSsh + "': ignoring device '" + inDevice.name + "'");
