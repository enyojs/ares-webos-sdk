	...
	novacom.Session.prototype = {
		...
		begin: function(target, next) {
			...
			if (!this.ssh) {
				...
				//test byJunil - add "webos"
				//this.target.passphrase = "webos";
				//this.target.password = "webos";
				//console.log("[ByJunil] target:",target);
		...
		run: function(cmd, stdin, stdout, stderr, next) {
			...
			this.ssh.exec(cmd, (function(err, stream) {
				console.log('Session#run()', 'exec cmd: ' + cmd + ', err:' + err);
		 ...
		 * @param {Function} next commonJS callback invoked upon completion or failure
		 */
		runWithNoReturn: function(cmd, stdin, stdout, stderr, next) {
			var err;
			log.verbose('Session#run()', "cmd=" + cmd);
			if (typeof next !== 'function') {
				throw new Error("BUG: 'next' is not a callback");
		 ...
		 * @param {Function} next commonJS callback invoked upon completion or failure
		 */
		forward: function(deviceAddr, devicePort, localPort, next) {
			...
			var localServer = net.createServer(function(inCnx) {
				...
				// client.
				session.ssh.forwardOut("127.0.0.1" /*srcAddr*/, inCnx.remotePort /*srcPort*/, deviceAddr /*dstAddr*/, devicePort /*dstPort*/, function(err, outCnx) {
					if (err) {
			...
			try {
				...
				localServer.listen(localPort, "127.0.0.1", null, (function(){
					...
					session.forwardedPorts.push({local:localServerPort, device: devicePort});
					next();	
				}.bind(this)));
			} catch(err) {
				next(err);
		...
		getLocalPort:function(remotePort){
			var session = this;
			var found = null;
