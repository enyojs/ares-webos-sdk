	...
	novacom.Resolver.prototype = {
		...
		_loadOne: function(inDevice, next) {
			...
			if (typeof inDevice.privateKey === 'string') {
				...
				var builtinPrvKeyFile = path.join(__dirname, inDevice.privateKey.openSsh);
				var prvKeyFile = path.join(keydir, inDevice.privateKey.openSsh);
				async.waterfall([
					function(next) {
						fs.exists(builtinPrvKeyFile, function(exist) {
							setImmediate(next, null, exist);
						});
					},
					function(builtinKeyExist, next) {
						if (builtinKeyExist) 
							shelljs.cp('-rf', builtinPrvKeyFile, keydir);
						setImmediate(next);
					},
					fs.readFile.bind(this, prvKeyFile, next),
