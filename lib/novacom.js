...
(function() {
	...
	novacom.Resolver.prototype = {
			...
			function _replaceDeviceFile(srcPath, dstPath, next) {
				...
				fs.stat(dstPath, function(err, dstStat) {
					if (err) {
						...
						var devicesData = fs.readFileSync(dstPath);
						try {
							var devices = JSON.parse(devicesData);
							self._checkValidNovacomDevices(devices, function(err, result) {
								if (err || result && result.errors.length > 0) {
									_resetDeviceFileWithMsg(srcPath, dstPath, next);
								} else {
									// Use existing novacom-devices.json
									setImmediate(next, err, dstPath);
								}
							});
						} catch (err) {
							_resetDeviceFileWithMsg(srcPath, dstPath, next);
					...
					function _resetDeviceFileWithMsg(srcPath, dstPath, next) {
						log.warn("Resolver#_load#_checkFile() device registry format has been changed in cli");
						log.warn("Resolver#_load#_checkFile() initializing device registry...");
						log.warn("Resolver#_load#_checkFile() please register device info again");
		...
		_checkValidNovacomDevices: function(devices, next) {
		...
		save: function(devicesData, next) {
			...
			fs.exists(deviceFilePath, function(exist) {
				...
				async.waterfall([
					self._checkValidNovacomDevices.bind(self, devicesData)
				], function(err, result) {
					if (err)
						return next(err);
					else {
