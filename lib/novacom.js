				...
				function(target, next) {
					...
					var req = http.request(options, function(res) {
						...
						if (res.statusCode === 404) {
							setImmediate(next, new Error("(404) Failed to get ssh private key")); 
						} else {
					...
					req.on('error', function(){ 
						setImmediate(next, new Error("Failed to get ssh private key")); 
					});
					req.end();
				},
			...
			this.ssh.exec(cmd, (function(err, chStream) {
				...
				if (stdin) {
					stdin.on('data', function(data) {
						if (chStream.writable && chStream.writable === true) { 
							if (chStream.write(data) === false) {
								stdin.pause();
								// WORKAROUND
								// ssh2 still does not emit drain event properly
								// so regulary call resume() here
								setTimeout(function() {stdin.resume();clearTimeout();}, 1000);
							}
						}
					});
		...
		runNoHangup: function(cmd, cbExit, next) {
			...
			this.ssh.exec(cmd, (function(err, stream) {
				...
				stream.on('data', function(data, extended) {
					extended = extended || 'stdout';
					log.verbose('[Session#runNoHangup()#onData]', data);
				});
