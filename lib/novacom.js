...
(function () {
	...
	function Resolver() {
		 ...
		 */
		// FIXME: all this stuff should go in .ssh/config
		this.devices = [
			{
				...
				port: 5522,
				username: 'root',
				privateKey: fs.readFileSync(path.join(keydir, 'webos')), // FIXME trap missing webos key
			...
			},
			{
				...
				host: '192.168.56.101', // FIXME: need an options or a configuration file
				port: 22,
				username: 'root',
				privateKey: fs.readFileSync(path.join(keydir, 'webos')),
				debug: null //console.log //log.verbose.bind(null, 'ssh')
	...
	function Session(target, next) {
		if (!next) {
			// FIXME: unclear error message
			throw new Error('novacom.Session() demands an availability common-js callback');
		}
		...
		begin: function(next) {
			if (!this.ssh) {
				...
				this.ssh.on('connect', function() {
					log.verbose('Session()', "ssh session event: connected");
				});
				...
				this.ssh.on('end', function() {
					log.verbose('Session()', "ssh session event: end");
				});
				this.ssh.on('close', function(had_error) {
					log.verbose('Session()', "ssh session event: close  (had_error:", had_error, ")");
				});
