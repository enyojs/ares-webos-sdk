	...
	novacom.Resolver.prototype = {
		...
		load: function(next) {
			...
			function _replaceDeviceFile(srcPath, dstPath, next) {
				log.verbose("Resolver#load#_checkFile()", "buildtin:", srcPath, ",others:", dstPath);
					...
					if (err) {
					 	if (err.code === 'ENOENT') {
							async.series([
								...
								if (err) {
									this.deviceFilePath = srcPath;
									return setImmediate(next, err);
								}
								this.deviceFilePath = dstPath;
								setImmediate(next, null, this.deviceFilePath);
							});
						} else {
							return setImmediate(next, err);
						}
					 } else {
					 	//check necessary of replacement with new one
					 	//1. compare modification date (if dstPath file is older than srcPath one, it should be replaced) 
		...
		_loadOne: function(inDevice, next) {
			...
			if (typeof inDevice.privateKey === 'string') {
				...
				setImmediate(next);
			} else if (inDevice.host === '127.0.0.1' && inDevice.port === '6622') {
		...
		save: function(devicesData, next) {
			...
			if (!this.deviceFilePath) {
			    this.deviceFilePath = path.join(appdir, 'novacom-devices.json');
				fs.exists(this.deviceFilePath, function(exist) {
					if(!exist) {
			    		this.deviceFilePath = path.join(__dirname, 'novacom-devices.json');
					}
				}.bind(this));
			}
			fs.writeFile(this.deviceFilePath, JSON.stringify(devicesData, null, "\t"),next);
		},		
	...
	novacom.Session.prototype = {
		...
		begin: function(target, next) {
			...
			if(!this.target.privateKey) {
				setImmediate(next, new Error("Private Key does not exist!!"));
				return;
			}
