...
(function() {
	...
	var gdbserver = {
		...
		run:function(options, params, next) {
			...
			function _getEnv(next) {
				log.verbose("gdbserver#run()#_getEnv");
				if (this.session.getDevice().username != 'root') {
					setImmediate(next, null, "");
					return;
				}
				var cmdGetEnvVars = "find /etc/jail_native_devmode.conf | xargs awk '/setenv/{printf \"export %s=%s;\\n\", $2,$3}'";
				var strEnvVars = "";
			...
			function _launchGdbserver(prefixEnv, next) {
				...
				//options.session.runNoHangup(cmdRunGdbserver, __exit, next);
				//options.session.runNoHangup("set", __exit, next);
