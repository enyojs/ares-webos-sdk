	...
	var gdbserver = {
		...
		run: function(options, params, next) {
				...
				if (options.instPkgs) {
					...
					options.instPkgs.every(function(pkg) {
						if (id.indexOf(pkg.id) !== -1) {
							prefixAppPath = prefixServicePath = path.join(path.dirname(pkg.folderPath), '..', suffix).replace(path.sep, '/');
							return false;
						}
						return true;
					});
			...
			function _launchGdbserver(prefixEnv, next) {
				...
				var format = "gdbserver :%s %s";
				if (appId) {
					var cmdRunGdbserver = util.format(format, this.port, path.join(prefixAppPath, appId, this.execName));
				} else if (svcId) {
					var cmdRunGdbserver = util.format(format, this.port, path.join(prefixServicePath, svcId, this.execName));
				}
