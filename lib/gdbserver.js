	...
	var gdbserver = {
		...
		run:function(options, params, next) {
			...
			function _killPrevGdbserver(next) {
				log.verbose("gdbserver#run()#_killPrevGdbserver");
				var cmdKillGdbserver = "kill -9 `pidof gdbserver` 2>/dev/null";
				options.session.runNoHangup(cmdKillGdbserver, null, null, null, next);
			}
		...
		close:function(options, params, next) {
			...
			function _killPrevGdbserver(next) {
				log.verbose("gdbserver#close()#_killPrevGdbserver");
				var cmdKillGdbserver = "kill -9 `pidof gdbserver` 2>/dev/null";
				options.session.runNoHangup(cmdKillGdbserver, null, null, null, next);
			}
		},
