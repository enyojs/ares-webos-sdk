	...
	var gdbserver = {
		...
		run:function(options, params, next) {
			...
			function _launchGdbserver(prefixEnv, next) {
				...
				options.session.runNoHangup(prefixEnv + cmdRunGdbserver, null, null, null, next);
			}
		...
		killProcByPid:function(pid, next) {
			...
			function _killProc(pid, next) {
				...
				var cmdKillPid = sprintf(format, pid);
				this.session.runNoHangup(cmdKillPid, null, null, null, next);
			}
		},
