...
(function() {
	...
	var luna = {
		...
		send: function(options, addr, param, onResponse, next) {
			var novacom = options.novacom || new novacom.Session(null, next);
			...
			novacom.run("/usr/bin/luna-send -i " + url + " '" + JSON.stringify(param) + "'", is /*stdin*/, os /*stdout*/, es /*stderr*/, function(err) {
				if (err) {
