	...
	var launcher = {
		...
		launch: function(options, id, params, next) {
			...
			function _launch(next) {
				var deviceInfo = options.session.resolver.getDeviceBy('name', options.device || 'default');
				luna.send(options, {	// luna addr
					service: deviceInfo.lunaAppMgr || 'com.palm.applicationManager',
					method: 'launch'
				}, {	// luna param
					subscribe: false,
					id: id,
					...
					if(options.device !== 'tv'){
						lineObj.procId = lineObj.processId;
					}
					if (lineObj.procId) {
						// success: stop
						log.verbose("launcher#launch#_launch():", "success");
						next(null, { procId: lineObj.procId});
					} else {
						// failure: stop
